...
def is_loopback_host(host: str) -> bool:
    """True when ``host`` is a loopback name or loopback IP (IPv4/IPv6)."""
    name = str(host or "").strip().lower()
    if not name or name in _LOOPBACK_HOST_NAMES:
        return bool(name)
    try:
...
USAGE = """{prog} [--host HOST] [--port N | --ephemeral] [--new] [--json]
       {pad} [--dist DIR] [--api-only] [--no-registry] [--allow-remote-host]
       {prog} list [--json]
...
_HELP = {
    "host": "bind address (default: 127.0.0.1; non-loopback requires --allow-remote-host)",
    "port": (
    ...
    "allow_remote_host": (
        "permit a non-loopback --host (UNAUTHENTICATED; loopback is the trust boundary)"
    ),
}
...
def build_parser(prog: str = DEFAULT_PROG) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--allow-remote-host",
        dest="allow_remote_host",
        action="store_true",
        help=_HELP["allow_remote_host"],
    )
    return parser
...
def parse_args(argv: list[str], *, prog: str = DEFAULT_PROG) -> dict:
    ...
    return {
        ...
        "allow_remote_host": namespace.allow_remote_host,
    }
...
def serve(argv: list[str], *, prog: str = DEFAULT_PROG) -> int:
    ...
    if not is_loopback_host(args["host"]) and not args["allow_remote_host"]:
        _err(
            f"Refusing non-loopback --host {args['host']!r}: the CAD Viewer is "
            "unauthenticated and loopback is the trust boundary. "
            "Pass --allow-remote-host only if you understand the risk "
            "(see SECURITY.md).\n"
        )
        return 1
