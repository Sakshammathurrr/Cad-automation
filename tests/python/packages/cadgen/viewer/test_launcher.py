...
class ArgumentGrammar(unittest.TestCase):
    ...
    def test_the_three_dev_flags_default_off_and_are_independent(self) -> None:
        defaults = self.parse([])
        for flag in ("ephemeral", "no_registry", "api_only", "allow_remote_host"):
            self.assertFalse(defaults[flag], flag)
        for argument, key in (
            ("--ephemeral", "ephemeral"),
            ("--no-registry", "no_registry"),
            ("--api-only", "api_only"),
            ("--allow-remote-host", "allow_remote_host"),
        ):
            args = self.parse([argument])
            self.assertTrue(args[key], argument)
            others = {"ephemeral", "no_registry", "api_only", "allow_remote_host"} - {key}
            for other in others:
    ...
    def test_loopback_hosts_are_recognized(self) -> None:
        for host in ("127.0.0.1", "localhost", "::1", "127.0.0.2"):
            self.assertTrue(main_module.is_loopback_host(host), host)
        self.assertFalse(main_module.is_loopback_host("0.0.0.0"))
        self.assertFalse(main_module.is_loopback_host("192.168.1.10"))

    def test_non_loopback_host_is_refused_without_allow_remote_host(self) -> None:
        stderr = io.StringIO()
        with contextlib.redirect_stderr(stderr):
            code = main_module.serve(["--host", "0.0.0.0", "--json"])
        self.assertEqual(code, 1)
        message = stderr.getvalue()
        self.assertIn("Refusing non-loopback", message)
        self.assertIn("--allow-remote-host", message)

    def test_allow_remote_host_parses_with_non_loopback_host(self) -> None:
        args = self.parse(["--host", "0.0.0.0", "--allow-remote-host"])
        self.assertEqual(args["host"], "0.0.0.0")
        self.assertTrue(args["allow_remote_host"])
